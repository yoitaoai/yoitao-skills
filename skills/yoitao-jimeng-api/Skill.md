---
name: yoitao-jimeng-api_使用即梦生成图片
description: Generate images，如果用户要求生产图片，没有指明哪个 skill 或者方式的时候，默认用这个 skill 来生成图片。using the Jimeng API based on text prompts. Use this skill when users request AI-generated images from the Jimeng (即梦AI) service, artwork, illustrations, or visual content creation. Supports text-to-image and image-to-image generation with customizable ratios and resolutions. 一次只能生成一张。
version: 1.0.0
...
1. **检查 Docker 是否已安装并可用**
   - 运行：`docker --version`
   - 建议再检查 daemon：`docker info >/dev/null`
   - 如果命令不存在或 daemon 未启动：先提示用户安装/启动 Docker（例如 Docker Desktop），不要继续后续生成流程

2. **检查本技能的 Docker Compose 服务是否已启动**
   - Compose 文件：`skills/yoitao-jimeng-api/scripts/docker-compose.yml`
   - 先检查：`docker compose -f skills/yoitao-jimeng-api/scripts/docker-compose.yml ps`
   - 若 `jimeng-api` 服务已经在运行（`running` / `healthy`），则继续后续流程
   - 若未启动，则先启动：`docker compose -f skills/yoitao-jimeng-api/scripts/docker-compose.yml up -d`

3. **确认 API 可访问（建议）**
   - 检查：`curl -fsS http://localhost:5100/ping`
   - 成功后再继续 Session ID 获取和图片生成

**Region-specific prefixes**:

- 国内站: Direct sessionid (e.g., `f5849b5a4c6f11aa3777592c1bfd8cc6`)
...
- **如果用户未提供 sessionId**：自动按照同目录下文档 `登录即梦获取cookie.md` （通过 Playwright 打开即梦网站获取 cookie）来获取，**不要使用任何硬编码的默认值**
- **如果用户主动提供了 sessionId**：直接使用用户提供的值
- **如果用户需要使用国际站**：要求用户提供带前缀的 sessionId（us-/hk-/jp-/sg-）
...
2. **检查 Docker / Jimeng API 服务状态（必须）**：
   - 先检查 Docker 是否已安装且可用（`docker --version` / `docker info`）
   - 再检查 `skills/yoitao-jimeng-api/scripts/docker-compose.yml` 对应服务是否已启动
   - 已启动则继续；未启动则先执行 `docker compose -f skills/yoitao-jimeng-api/scripts/docker-compose.yml up -d`
   - 确认 `http://localhost:5100/ping` 可访问后再继续
3. **获取 Session ID**：如果用户未提供，调用 `jimeng-sessionid` skill 自动获取（不要使用硬编码默认值）
...
5. **Execute generation** using the `generate_image.py` script — REMINDER: **only pass parameters explicitly requested by the user; do not add/guess any optional flags**
6. **Report results** — show file paths only. **DO NOT READ/OPEN/ANALYZE GENERATED IMAGES. DO NOT CALL ANY READ TOOL (e.g., `Read`, `view_image`). STOP AFTER SAVING.**
...
User requests image generation
    ↓
Is Docker installed and daemon running?
    ├─ No → 提示用户先安装/启动 Docker（如 Docker Desktop）→ 停止
    └─ Yes → Continue
    ↓
Is `skills/yoitao-jimeng-api/scripts/docker-compose.yml` service (`jimeng-api`) running?
    ├─ No → Run `docker compose -f skills/yoitao-jimeng-api/scripts/docker-compose.yml up -d`
    │       → Verify `http://localhost:5100/ping` → Continue
    └─ Yes → Continue
    ↓
Do we have Session ID?
    ├─ No → 调用 jimeng-sessionid skill 自动获取 → 获取成功后继续
    │       （如果获取失败，提示用户手动提供）
    └─ Yes → Continue
    ↓
...
- 调用 `jimeng-sessionid` skill 自动获取
- 如果自动获取失败，提示用户手动提供
- 国际站需要用户提供带前缀的 sessionId（us-/hk-/jp-/sg-）
...
1. **自动获取 Session ID** - 用户未提供时调用 `jimeng-sessionid` skill 自动获取，不要使用硬编码默认值
...
**Example 1: Simple text-to-image (自动获取 sessionId)**

```
User: "用即梦生成一张日落山景图"
Claude: [调用 jimeng-sessionid skill 自动获取 sessionId]
Claude: [使用获取到的 sessionId 执行 generate_image.py text mode]
Claude: "✅ 成功生成4张图片!
