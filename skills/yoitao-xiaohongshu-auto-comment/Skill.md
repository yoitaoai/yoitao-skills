...
- 每次运行后将已发送评论追加保存到日志文件 `自媒体/AI自动运营/小红书_comments_log.json`。
- 下次运行时加载历史记录，确保新评论与最近 50 条历史评论不重复（语义相似度不能过高）。
...
读取 `自媒体/AI自动运营/小红书_comments_log.json`（如不存在则初始化为空数组）。提取最近 50 条评论用于去重校验。
